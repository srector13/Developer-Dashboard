...
/// Translate the legacy accelerator spellings ("CommandOrControl+Shift+Space",
/// "CmdOrCtrl+G") into the form tauri-plugin-global-shortcut parses, so a
/// settings.json written by hand — or carried over from an older build, or
/// copied from Markdown Notebook — keeps working.
...
mod tests {
    ...
    #[test]
    fn legacy_accelerator_spellings_are_translated() {
        assert_eq!(
