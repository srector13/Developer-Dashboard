...
// `model` and `search` used to live here. They are `suite-core` now — a log
// source is an item with a "tail this" action, so the vocabulary was never
// Dev Hub's to keep.
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Announce ourselves to the rest of the suite before anything else,
            // so a sibling launched a second later can already find us.
